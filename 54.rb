...
class Hand
	...
	def <=>(hand2)
      ...
      deltas = cards1.group_by {|c|c.val} \          # group the cards by val
								.sort_by {|c,g| -g.length}           # largest set first (for full house)
								.map(&:first) \											 # take just the number (only needed the length for sorting)
								.deltas(                             # compute the deltas between this and the second group of cards
									cards2.group_by {|c|c.val} \       # same as group1
										.sort_by { |c,g| -g.length } \   # same as group1
										.map(&:first) \                  # same as group1
								 	) \
								.select{|d|d!=0} \                   # filter out the equal cards
								.reverse                             
			if deltas.length > 0
