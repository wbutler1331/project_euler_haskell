...
findSeries = worker [] 1000
	where
		worker _ 9999 = []
		worker alreadyChecked currentNumber
			| currentNumber `elem` alreadyChecked = worker alreadyChecked nextNumber
			| otherwise           = answers : worker (perms ++ alreadyChecked) nextNumber
			where
				answers =  [[x,y,z] | x <- perms, 
									 y <- perms, 
									 z <- perms, 
									 y-x == z-y,
									 x /= y,
									 y /= z,
									 x /= z,
									 all (\n -> millerRabinPrimality n 592) [x,y,z]]
				perms = filter (>1000) $ map toInt $ permutations strn
